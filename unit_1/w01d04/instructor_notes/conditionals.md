...
- `if` color equals `green` is `true` throw it in the trash
- `else` eat it

If we were to code it, we might write

let chosenMMColor = 'green';
...
1) Change the value of your `hungry` variable to `!true`. Log `hungry` and run your code to see the resulting value.

2) Using a bang (`!`), change the value of `thirsty` to change the value to true. Log `thirsty` to see the value.

3) Another use case for toggling would be determining whose turn it is in a game. Let's say you are playing a game of chess against a computer
...
[Here is a great table comparing and contrasting results of](https://dorey.github.io/JavaScript-Equality-Table/) `==`, `===` and `if()` (use tabs along the top)
...
The **modulo** operator (also referred to as modulus) is represented as a `%` in JavaScript. The Modulo returns the remainder of Euclidean division.
...
```
cars = ["Chevy", "Audi", "Mazda", "Toyota", "Lexus", "Tesla"];
...
- **Array** (using the car array we declared above)
...
Compared to the for loop we wrote above, this syntax is shorter. However we don't, have access to the index number, so this is more useful for when we know we want to iterate over all the times.
